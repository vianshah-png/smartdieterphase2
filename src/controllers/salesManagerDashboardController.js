...
import { getCounsellorPerformance } from "../controllers/salesDashboardControllers/overviewController.js";
...
const salesBreakdown = async (req, res, next) => {
  try {
    ...
    const counts = await readRecordUnion([
      ...
      {
        selectField: [
          "COUNT(DISTINCT sop.sub_order_id) AS units",
          "SUM(od.order_paid_amount + od.order_balance_amount) AS revenue",
          "'total_today' AS type",
        ],
        table: `${tables.orderDetails} od`,
        join: [
          { type: "INNER", table: `${tables.userDetails} cd`, on: "cd.user_id = od.user_id" },
          { type: "INNER", table: `${tables.subOrderPrograms} sop`, on: "sop.order_id = od.order_id" },
        ],
        condition: [
          { field: "od.order_date", operator: "BETWEEN", value: [todayStart, todayEnd] },
          { field: "od.order_type", operator: "IN", value: ["New","OCR"] },
        ],
      },
      {
        selectField: [
        ...
        condition: [
          { field: "od.order_date", operator: "BETWEEN", value: [monthStart, now] },
          { field: "od.order_type", operator: "IN", value: ["New","OCR"] },
        ],
      },
      ...
      // ================= FRESH LEADS =================
      {
        ...
        condition: [
          { field: "od.order_date", operator: "BETWEEN", value: [todayStart, todayEnd] },
          { field: "od.order_type", operator: "=", value: "New" },
          { field: "cd.added_date", operator: ">=", value: monthStart },
        ],
      },
      {
        selectField: [
    ...
    const formatted = {
      total: { today: {}, mtd: {} },
      fresh_leads: { today: {}, mtd: {} },
      old_leads: { today: {}, mtd: {} },
      old_clients: { today: {}, mtd: {} },
    };

    counts.forEach((item) => {
      const [category, period] = item.type.split("_");
      const map = {
        total: "total",
        fresh: "fresh_leads",
        old: "old_leads",
    ...
    return res.status(200).json(
      new ApiResponse({
        statusCode: 200,
        message: "Sales breakdown (Today + MTD) fetched successfully",
        data: formatted,
      })
    );

  } catch (error) {
    console.log("Error while fetching sales breakdown", error);
    return next(new ErrorHandler("Internal server error", 500));
  }
};

const leadsBreakdown = async (req, res, next) => {
  try {
    const { mentor_id } = req.query;
    ...
    const counts = await readRecordUnion([
      // ================= TOTAL LEADS =================
      {
        selectField: ["COUNT(DISTINCT cd.user_id) AS count", "'total_today' AS type"],
        table: `${tables.userDetails} cd`,
        join: [{ type: "INNER", table: `${tables.leadAssignedLog} lal`, on: `cd.user_id = lal.user_id` }],
        condition: [
          { field: "cd.user_type", operator: "=", value: "0" },
          { field: "DATE(lal.assign_date)", operator: "=", value: "CURDATE()", raw: true },
        ],
      },
      {
        selectField: ["COUNT(DISTINCT cd.user_id) AS count", "'total_mtd' AS type"],
        table: `${tables.userDetails} cd`,
        join: [{ type: "INNER", table: `${tables.leadAssignedLog} lal`, on: `cd.user_id = lal.user_id` }],
        condition: [
          { field: "cd.user_type", operator: "=", value: "0" },
          { field: "DATE(lal.assign_date)", operator: ">=", value: "DATE_FORMAT(CURDATE(), '%Y-%m-01')", raw: true },
        ],
      },

      // ================= TM LEADS =================
      {
        selectField: ["COUNT(DISTINCT cd.user_id) AS count", "'tm_today' AS type"],
        table: `${tables.userDetails} cd`,
        join: [
          { type: "INNER", table: `${tables.leadAssignedLog} lal`, on: `cd.user_id = lal.user_id` },
          {
            type: "LEFT",
            table: `(SELECT user_id, MAX(created) as latest_created
                     FROM bn_client_hs
          ...
          { type: "LEFT", table: `bn_client_hs hs`, on: "hs.user_id = latest_hs.user_id AND hs.created = latest_hs.latest_created" },
        ],
        condition: [
          { field: "cd.user_type", operator: "=", value: "0" },
          { field: "DATE(lal.assign_date)", operator: "=", value: "CURDATE()", raw: true },
          {
            orConditions: [
              { field: "ABS(hs.weight_difference) >= 7", operator: "", value: "", raw: true },
              { field: "JSON_LENGTH(cd.health_conditions) > 0", operator: "", value: "", raw: true },
              { field: "JSON_LENGTH(hs.health_issue) > 0", operator: "", value: "", raw: true },
              { field: "cd.country_id != 101", operator: "", value: "", raw: true },
            ],
          },
        ],
      },
      {
        selectField: ["COUNT(DISTINCT cd.user_id) AS count", "'tm_mtd' AS type"],
        table: `${tables.userDetails} cd`,
        join: [
          { type: "INNER", table: `${tables.leadAssignedLog} lal`, on: `cd.user_id = lal.user_id` },
          {
      ...
      // ================= CONSULTATIONS =================
      {
        selectField: ["COUNT(DISTINCT cu.user_id) AS count", "'consult_today' AS type"],
        table: `${tables.callUpdates} cu`,
        condition: [
          { field: "cu.call_type", operator: "=", value: "30" },
          { field: "DATE(cu.schedule_date)", operator: "=", value: "CURDATE()", raw: true },
        ],
      },
      {
        selectField: ["COUNT(DISTINCT cu.user_id) AS count", "'consult_mtd' AS type"],
        table: `${tables.callUpdates} cu`,
        condition: [
          { field: "cu.call_type", operator: "=", value: "30" },
          { field: "DATE(cu.schedule_date)", operator: ">=", value: "DATE_FORMAT(CURDATE(), '%Y-%m-01')", raw: true },
        ],
      },
      ...
      {
        selectField: ["COUNT(DISTINCT cd.user_id) AS count", "'yet_capture' AS type"],
        table: `${tables.userDetails} cd`,
        condition: [
          { field: "cd.user_type", operator: "=", value: "0" },
          { field: "cd.counsellor_assigned", operator: "IS", value: "NULL", raw: true },
          { field: "DATE(cd.added_date)", operator: ">=", value: "DATE_FORMAT(CURDATE(), '%Y-%m-01')", raw: true },
        ],
      },
      {
        selectField: ["COUNT(DISTINCT cd.user_id) AS count", "'yet_engage' AS type"],
        table: `${tables.userDetails} cd`,
        condition: [
          { field: "cd.sales_status", operator: "=", value: "0" },
          { field: "DATE(cd.added_date)", operator: ">=", value: "DATE_FORMAT(CURDATE(), '%Y-%m-01')", raw: true },
      ...
      {
        selectField: ["COUNT(DISTINCT cd.user_id) AS count", "'yet_consult' AS type"],
        table: `${tables.userDetails} cd`,
        join: [{ type: "LEFT", table: `${tables.callUpdates} cu`, on: "cd.user_id = cu.user_id AND cu.call_type = 30" }],
        condition: [
    ...
    counts.forEach((item) => {
      if (item.type.startsWith("yet_")) {
        // Single count metrics
        formatted[item.type] = parseInt(item.count) || 0;
      } else {
        const period = item.type.endsWith("_today") ? "today" : "mtd";
        const key = item.type.replace("_today", "").replace("_mtd", "");
    
        if (!formatted[key]) {
          formatted[key] = { today: 0, mtd: 0 };
        }
    
        formatted[key][period] = parseInt(item.count) || 0;
      }
    });
    ...
    return res.status(200).json(
      new ApiResponse({
        statusCode: 200,
        message: "Leads breakdown fetched successfully",
        data: formatted,
      })
    );
  } catch (error) {
    console.log("Error while fetching leads breakdown", error);
    return next(new ErrorHandler("Internal server error", 500));
  }
};
...
const leadDistribution = async (req, res, next) => {
  try {
    ...
    const counts = await readRecordUnion([
      ...
      // Doctors
      {
        selectField: ["COUNT(DISTINCT cd.user_id) AS count", "'doctors_today' AS type"],
        table: `${tables.userDetails} cd`,
        join: [{
          type: "INNER",
          table: `${tables.leadSource} ls`,
        ...
        condition: [
          { field: "cd.user_type", operator: "=", value: "0" },
          { field: "ls.source_group", operator: "=", value: "7" },
          { field: "DATE(cd.added_date)", operator: "=", value: "CURDATE()", raw: true },
        ],
      },
      {
        selectField: ["COUNT(DISTINCT cd.user_id) AS count", "'doctors_mtd' AS type"],
        table: `${tables.userDetails} cd`,
        join: [{
          type: "INNER",
          table: `${tables.leadSource} ls`,
      ...
      // Corporates
      {
        selectField: ["COUNT(DISTINCT cd.user_id) AS count", "'corporates_today' AS type"],
        table: `${tables.userDetails} cd`,
        join: [{
          type: "INNER",
          table: `${tables.leadSource} ls`,
        ...
        condition: [
          { field: "cd.user_type", operator: "=", value: "0" },
          { field: "ls.source_group", operator: "=", value: "8" },
          { field: "DATE(cd.added_date)", operator: "=", value: "CURDATE()", raw: true },
        ],
      },
      {
        selectField: ["COUNT(DISTINCT cd.user_id) AS count", "'corporates_mtd' AS type"],
        table: `${tables.userDetails} cd`,
        join: [{
          type: "INNER",
          table: `${tables.leadSource} ls`,
      ...
      // Nutripreneur
      {
        selectField: ["COUNT(DISTINCT cd.user_id) AS count", "'nutripreneur_today' AS type"],
        table: `${tables.userDetails} cd`,
        join: [{
          type: "INNER",
          table: `${tables.leadSource} ls`,
        ...
        condition: [
          { field: "cd.user_type", operator: "=", value: "0" },
          { field: "ls.source_group", operator: "=", value: "9" },
          { field: "DATE(cd.added_date)", operator: "=", value: "CURDATE()", raw: true },
        ],
      },
      {
        selectField: ["COUNT(DISTINCT cd.user_id) AS count", "'nutripreneur_mtd' AS type"],
        table: `${tables.userDetails} cd`,
        join: [{
          type: "INNER",
          table: `${tables.leadSource} ls`,
      ...
      {
        selectField: ["COUNT(DISTINCT cd.user_id) AS count", "'sm_today' AS type"],
        table: `${tables.userDetails} cd`,
        join: [{ type: "INNER", table: `${tables.leadSource} ls`, on: "cd.current_lead_source = ls.source_id"}],
        condition: [
          { field: "cd.user_type", operator: "=", value: "0" },
          { field: "ls.source_group", operator: "=", value: "3" },
          { field: "DATE(cd.added_date)", operator: "=", value: "CURDATE()", raw: true },
      ...
      {
        selectField: ["COUNT(DISTINCT cd.user_id) AS count", "'sm_mtd' AS type"],
        table: `${tables.userDetails} cd`,
        join: [{ type: "INNER", table: `${tables.leadSource} ls`, on: "cd.current_lead_source = ls.source_id"}],
        condition: [
          { field: "cd.user_type", operator: "=", value: "0" },
      ...
      // Health Score (group 1)
      {
        selectField: ["COUNT(DISTINCT cd.user_id) AS count", "'hs_today' AS type"],
        table: `${tables.userDetails} cd`,
        join: [{ type: "INNER", table: `${tables.leadSource} ls`, on: "cd.current_lead_source = ls.source_id"}],
        condition: [
          { field: "cd.user_type", operator: "=", value: "0" },
      ...
      {
        selectField: ["COUNT(DISTINCT cd.user_id) AS count", "'hs_mtd' AS type"],
        table: `${tables.userDetails} cd`,
        join: [{ type: "INNER", table: `${tables.leadSource} ls`, on: "cd.current_lead_source = ls.source_id"}],
        condition: [
          { field: "cd.user_type", operator: "=", value: "0" },
      ...
      // App (group 10)
      {
        selectField: ["COUNT(DISTINCT cd.user_id) AS count", "'app_today' AS type"],
        table: `${tables.userDetails} cd`,
        join: [{ type: "INNER", table: `${tables.leadSource} ls`, on: "cd.current_lead_source = ls.source_id"}],
        condition: [
          { field: "cd.user_type", operator: "=", value: "0" },
          { field: "ls.source_group", operator: "=", value: "10" },
          { field: "DATE(cd.added_date)", operator: "=", value: "CURDATE()", raw: true },
        ],
      },
      {
        selectField: ["COUNT(DISTINCT cd.user_id) AS count", "'app_mtd' AS type"],
        table: `${tables.userDetails} cd`,
        join: [{ type: "INNER", table: `${tables.leadSource} ls`, on: "cd.current_lead_source = ls.source_id"}],
        condition: [
          { field: "cd.user_type", operator: "=", value: "0" },
      ...
      // ================= OL (OLD LEAD REASSIGNMENT) =================

      {
        selectField: ["COUNT(DISTINCT cd.user_id) AS count", "'ol_today' AS type"],
        table: `${tables.userDetails} cd`,
        join: [{ type: "INNER", table: `${tables.leadAssignedLog} lal`, on: "cd.user_id = lal.user_id"}],
        condition: [
          { field: "cd.user_type", operator: "=", value: "0" },
          { field: "YEAR(cd.added_date)", operator: "=", value: "YEAR(CURDATE())", raw: true },
          { field: "MONTH(cd.added_date)", operator: "<", value: "MONTH(CURDATE())", raw: true },
          { field: "DATE(lal.assign_date)", operator: "=", value: "CURDATE()", raw: true },
        ],
      },

      {
        selectField: ["COUNT(DISTINCT cd.user_id) AS count", "'ol_mtd' AS type"],
        table: `${tables.userDetails} cd`,
        join: [{ type: "INNER", table: `${tables.leadAssignedLog} lal`, on: "cd.user_id = lal.user_id"}],
        condition: [
          { field: "cd.user_type", operator: "=", value: "0" },
    ...
    counts.forEach(item => {
      ...
      if (!formatted[key]) formatted[key] = { today: 0, mtd: 0 };
      formatted[key][period] = parseInt(item.count) || 0;
    });
    ...
    console.log("Error fetching lead distribution", error);
    return next(new ErrorHandler("Internal server error", 500));
  }
};


const missesDashboard = async (req, res, next) => {
  try {
    ...
    if (userIds.length > 0) {
      ...
      unanswered_queries = results.length;
    }

    formatted.queries = unanswered_queries;
    ...
    return res.status(200).json(
      new ApiResponse({
        statusCode: 200,
        message: "Misses dashboard fetched successfully",
        data: formatted,
      })
    );

  } catch (error) {
    console.log("Error fetching misses dashboard", error);
    return next(new ErrorHandler("Internal server error", 500));
  }
};


const risksDashboard = async (req, res, next) => {
  try {
    ...
    const counts = await readRecordUnion([
      ...
      // ================= PAYMENT OD =================
      {
        selectField: [
          ...
          "'payment_od_today' AS type"
        ],
        table: `${tables.userDetails} ud`,
        condition: [
          { field: "ud.user_type", operator: "=", value: "0" },
          { field: "DATE(ud.payment_date)", operator: "=", value: "CURDATE()", raw: true },
          { field: "ud.payment_date", operator: "<", value: "NOW()", raw: true }
        ]
      },
      {
        selectField: [
          "COUNT(DISTINCT ud.user_id) AS count",
          "'payment_od_mtd' AS type"
        ],
        table: `${tables.userDetails} ud`,
        condition: [
          { field: "ud.user_type", operator: "=", value: "0" },
          {
            field: "DATE(ud.payment_date)",
            operator: ">=",
            value: "DATE_FORMAT(CURDATE(),'%Y-%m-01')",
            raw: true
          },
      ...
      // ================= PAYMENT LINK EXPIRED =================
      {
        ...
        condition: [
          { field: "ud.user_type", operator: "=", value: "0" },
          { field: "DATE(sp.payment_expiry)", operator: "=", value: "CURDATE()", raw: true },
          { field: "sp.payment_expiry", operator: "<", value: "NOW()", raw: true }
        ]
      },
      {
        selectField: [
          "COUNT(DISTINCT ud.user_id) AS count",
          "'payment_link_mtd' AS type"
        ],
        table: `${tables.userDetails} ud`,
        join: [
          {
      ...
      // ================= HOT DOWNGRADE (2 -> 3) =================
      {
        selectField: [
          "COUNT(DISTINCT ud.user_id) AS count",
          "'hot_downgrade_today' AS type"
        ],
        table: `${tables.userDetails} ud`,
        join: [
          {
            type: "INNER",
            table: `${tables.leadSaleStatusLog} ls`,
        ...
        condition: [
          ...
          { field: "JSON_EXTRACT(ls.sales_status_log,'$[0].sales_status')", operator: "=", value: "3", raw: true },
          { field: "JSON_EXTRACT(ls.sales_status_log,'$[1].sales_status')", operator: "=", value: "2", raw: true },

          {
            field: "DATE(STR_TO_DATE(JSON_UNQUOTE(JSON_EXTRACT(ls.sales_status_log,'$[0].timestamp')),'%Y-%m-%dT%H:%i:%s.%fZ'))",
            operator: "=",
            value: "CURDATE()",
            raw: true
          }
      ...
      // ================= WARM DOWNGRADE (3 -> 4) =================
      {
        ...
        condition: [
          ...
          { field: "JSON_EXTRACT(ls.sales_status_log,'$[0].sales_status')", operator: "=", value: "4", raw: true },
          { field: "JSON_EXTRACT(ls.sales_status_log,'$[1].sales_status')", operator: "=", value: "3", raw: true },
      ...
      // ================= PHASE 4 COLD =================
      {
        ...
        condition: [
          { field: "ud.user_type", operator: "=", value: "0" },
          { field: "ABS(hs.weight_difference)", operator: ">=", value: "7", raw: true },
          { field: "DATE(hs.created)", operator: "=", value: "CURDATE()", raw: true }
        ]
      },
      {
        selectField: [
    ...
    counts.forEach((item) => {
      const [key, period] = item.type.split("_");
    ...
    return res.status(200).json(
      new ApiResponse({
        statusCode: 200,
        message: "Risks dashboard fetched successfully",
        data: formatted
      })
    );

  } catch (error) {
    console.log("Error fetching risks dashboard", error);
    return next(new ErrorHandler("Internal server error", 500));
  }
};

const avgDaysToConvert = async (req, res, next) => {
  try {
    ...
    const counts = await readRecordUnion([
      ...
      {
        selectField: [
          "ROUND(AVG(DATEDIFF(fc.consult_date, ud.added_date)),1) AS value",
          "'lead_consult' AS type"
        ],
        table: `${tables.userDetails} ud`,
        join: [
          {
            type: "INNER",
            table: `(SELECT user_id, MIN(added_date) AS consult_date
                     FROM ${tables.consultationLogs}
                     GROUP BY user_id) fc`,
            on: "fc.user_id = ud.user_id",
            raw: true
          }
        ],
        condition: [
          { field: "fc.consult_date", operator: ">=", value: "ud.added_date", raw: true },
          { field: "ud.added_date", operator: ">=", value: "DATE_FORMAT(CURDATE(), '%Y-01-01')", raw: true }
        ]
      },
      ...
      {
        selectField: [
          "ROUND(AVG(DATEDIFF(fo.sale_date, fc.consult_date)),1) AS value",
          "'consult_sale' AS type"
        ],
        table: `${tables.userDetails} ud`,
        join: [
          {
          ...
          {
            type: "INNER",
            table: `(SELECT user_id, MIN(order_date) AS sale_date
                     FROM ${tables.orderDetails}
                     WHERE order_type='New'
                     GROUP BY user_id) fo`,
            on: "fo.user_id = ud.user_id",
            raw: true
          }
        ],
        condition: [
          { field: "fo.sale_date", operator: ">=", value: "fc.consult_date", raw: true },
          { field: "ud.added_date", operator: ">=", value: "DATE_FORMAT(CURDATE(), '%Y-01-01')", raw: true }
        ]
      },
      ...
      // ================= HOT → SALE =================
      {
          ...
          `ROUND(
            AVG(
              DATEDIFF(
                fo.sale_date,
                STR_TO_DATE(
                  JSON_UNQUOTE(JSON_EXTRACT(ls.sales_status_log,'$[0].timestamp')),
                  '%Y-%m-%dT%H:%i:%s.%fZ'
                )
              )
            ),1
        ...
        join: [
          {
            type: "INNER",
            table: `(SELECT user_id, MAX(id) AS latest_id
                     FROM ${tables.leadSaleStatusLog}
                     GROUP BY user_id) latest_log`,
            on: "latest_log.user_id = ud.user_id",
            raw: true
          },
          {
            type: "INNER",
            table: `${tables.leadSaleStatusLog} ls`,
            on: "ls.id = latest_log.latest_id"
          },
          {
            type: "INNER",
            table: `(SELECT user_id, MIN(order_date) AS sale_date
        ...
        condition: [
          {
            field: "JSON_UNQUOTE(JSON_EXTRACT(ls.sales_status_log,'$[0].sales_status'))",
            operator: "=",
            value: "2"
          },
          {
            field: "fo.sale_date",
            operator: ">=",
            value: "STR_TO_DATE(JSON_UNQUOTE(JSON_EXTRACT(ls.sales_status_log,'$[0].timestamp')),'%Y-%m-%dT%H:%i:%s.%fZ')",
            raw: true
          },
          {
            field: "ud.added_date",
            operator: ">=",
            value: "DATE_FORMAT(CURDATE(), '%Y-01-01')",
            raw: true
          }
        ]
      },
    ...
    counts.forEach(item => {
      formatted[item.type] = `${item.value || 0}d`;
    });

    return res.status(200).json(
      new ApiResponse({
        statusCode: 200,
        message: "Average days to convert fetched successfully",
        data: formatted
      })
    );

  } catch (error) {
    console.log("Error fetching avg days to convert", error);
    return next(new ErrorHandler("Internal server error", 500));
  }
};
